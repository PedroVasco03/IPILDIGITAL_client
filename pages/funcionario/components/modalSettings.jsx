...
function  ModalSetting({show, closed}){
    ...
    const getData = async ()=>{
        ...
        const value = localStorage.getItem('idfuncionario')
        await axios.get(`http://localhost:5000/funcionario/${value}`).then((res)=>{
    ...
    if(nome!="" && numbi!="" && telefone!="" && email !="" && senha !=""){
        await axios.patch(`http://localhost:5000/funcionario/${data}`,{
            nome: nome,
            bi: numbi,
            telefone: telefone,
            email: email,
