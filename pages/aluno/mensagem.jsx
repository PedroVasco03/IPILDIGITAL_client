...
function Chat() {
  ...
  const saverReceptor = async () => {
    const response = await axios.get('http://localhost:5000/enviado');
    const search = response.data.filter(data =>
      (data.nome === sender && data.receptor === receiver) ||
  ...
  const handleSendMessage = () => {
    if (message !== '') {
      ...
      setMessage('');
    } else alert('Mensagem não pode ser vazia!');
  };
          ...
          <div className={`col-md-8 col-xl-6 ${styleChat.chat_mensagens}`}>
            <div className={`card ${styleChat.chat_card}`}>
              ...
              <div className={`card-body ${styleChat.msgcard_body}`}>
                {messages.map((msg, idx) => msg.sender !== sender ? (
              ...
              <div className={styleChat.card_footer}>
                <div className="input-group">
                  ...
                  <span
                    ...
                    onClick={() => { saverReceptor(); handleGetMessages(); handleSendMessage(); }}
                  >
